...
router.post(
  '/signup',
  validateSchema('entities/user.entity.schema.json'),
  async (req, res, next) => {
    const { email, password, name, additionalName, birthDate, gender } =
      req.body;

    const existingUser = await User.findOne({ email });
    if (existingUser) {
      return next(
        new BadRequestError({ message: 'Email in use', prop: 'email' })
      );
    }
